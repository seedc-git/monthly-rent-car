...
- ページに必要な画像、ロゴ、スクリーンショット、参考素材は、依頼者から除外指定がない限り、公式サイトなどの一次ソースを優先して積極的に取得する。
- 取得した外部素材は原則としてリポジトリ内へ保存して使用し、出典URLと取得日を対象ページの監査資料へ記録する。
- 外部素材の取得を一律に見送り、プレースホルダー、AI生成画像、テキストだけへ置き換えない。取得対象から外すのは、依頼者から個別に除外・差し替えの指示を受けた素材だけとする。
