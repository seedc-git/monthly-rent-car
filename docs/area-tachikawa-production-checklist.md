...
- [ ] 18画像すべての使用素材が揃い、依頼者からの個別の除外・差し替え指示を反映した。
...
- [ ] `docs/area-tachikawa-ranking-assets.md`に記載した18スロットに使用画像を設定した。
- [ ] 必要な外部サイトの画像・ロゴを公式サイトなどの一次ソースから取得し、リポジトリ内へ保存した。
- [ ] 外部素材の取得元URLと取得日を記録し、依頼者から個別に指定された除外・差し替えは指定の代替素材へ差し替えた。
- [ ] `width`、`height`、`aspect-ratio`、`data-image-slot`が正しい。
- [ ] 画像内容に合わせてaltを確定し、装飾画像は空alt、情報として掲載する会社ロゴは会社名を示すaltにした。
