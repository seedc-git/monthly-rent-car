...
| テスト | 結果 |
|---|---|
| `node tools/check-tachikawa-ranking.cjs` | 合格。857 assertions |
...
| 表の固定列 | モバイルで横overflowする7表だけ左端のキー列をsticky固定。ランキング表は「レンタカー会社名」を1列だけ固定 |
| ランキング列 | 5列構成。`レンタカー会社名`、`おすすめ度`、`1ヶ月料金`、`立川での利用方法`、`主な特徴` |
| 横スクロール表の列幅 | 7表すべて途中列を内容に合わせて短縮し、最終列は固定列以外の画面幅を確保。右端では固定列と最終列が画面内に収まる構成 |
| 横スクロール表の見出し | 7表すべて薄いグレー背景・濃色文字・グレー罫線へ統一 |
